...
static void				ft_fill_bmp_header(t_bmp *bmp)
{
	...
	ft_memcpy(&(bmp->header[26]), &(bmp->color_planes), sizeof(uint16_t));
	ft_memcpy(&(bmp->header[28]), &(bmp->bpp), sizeof(uint16_t));
	ft_memcpy(&(bmp->header[34]), &(bmp->bufsize), sizeof(uint16_t));
